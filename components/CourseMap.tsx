...
const CourseMap: React.FC<CourseMapProps> = ({ courses, onSelectLesson, onBack }) => {
  ...
  return (
    <div className="space-y-8 animate-in slide-in-from-bottom duration-500 pb-12">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4 px-2 no-scrollbar">
        {courses.map((course) => (
          ...
          >
            ...
            <p className={`text-[10px] mt-2 text-left leading-tight ${selectedCourseId === course.id ? 'text-slate-200' : 'text-gray-400'}`}>
              {course.description}
            </p>
          </button>
        ))}
      ...
      {selectedCourse && (
        <div className="space-y-12 relative before:absolute before:left-[19px] before:top-4 before:bottom-4 before:w-0.5 before:bg-green-100 animate-in fade-in slide-in-from-top duration-300">
          {selectedCourse.modules.map((module, mIdx) => (
            <div key={module.id} className="relative z-10">
              <div className="flex items-center gap-4 mb-6">
                <div className="w-10 h-10 rounded-full bg-slate-800 border-4 border-white shadow-md flex items-center justify-center text-white text-xs font-bold">
                  {mIdx + 1}
                </div>
                <h2 className="text-lg font-extrabold text-slate-800 uppercase tracking-wider">{module.title}</h2>
              </div>
                ...
                {module.lessons.map((lesson) => (
                  ...
                  >
                    <div className="flex justify-between items-start">
                      ...
                      <div className={`w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0 ${lesson.completed ? 'bg-green-500 text-white' : 'bg-green-50 text-green-600'}`}>
